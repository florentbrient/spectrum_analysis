...
def compute_spectra(
    scalar_field, dx=1, periodic_domain=False,\
        apply_detrending=False, window=None,\
            fact=1
):
    """
    ...
    scalar_field : numpy array of shape (npx,npx) - npx is number of pixels
            (Cloud) scalar field.
            FB: Modified: List of variables (for E as UT^2+VT^2+WT^2)
    dx : int, optional
    ...
    psd_2d_all  =[]
    for ij in range(NS):
        N = np.min(scalar_field[ij].shape)
        
        # Detrending
        if apply_detrending:
            [X, Y] = np.meshgrid(
                np.arange(scalar_field[ij].shape[0]),
                np.arange(scalar_field[ij].shape[1]),
                indexing="ij",
            )
            scalar_field[ij], bDt = _detrend(scalar_field[ij], [X, Y])
    
        # Windowing
        if not periodic_domain:
            if window == "Planck":
                scalar_field[ij] = _planck_rad(scalar_field[ij])  # Planck-taper window
            elif window == "Welch":
                scalar_field[ij] = _welch_rad(scalar_field[ij])  # Welch window
            elif window == "Hann":
                scalar_field[ij] = _hann_rad(scalar_field[ij])  # Hann window
    
        # FFT
        Ftmp = fftpack.fft2(scalar_field[ij])  # 2D FFT (no prefactor)
